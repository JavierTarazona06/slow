...
class ConexionBaseDeDatosSlow():
    ...
    def abrirBasedeDatosSlow(self):
        datosConexion = open("conexionSlowDataBase.txt")
        infoConexion = datosConexion.read().split('\n')
        print(infoConexion)
        hostI = str(infoConexion[0])
        hostI1 = int(hostI.find('"')+1)
        hostI2 = int(len(hostI)-2)
        hostI = hostI[hostI1:hostI2]
        portI = str(infoConexion[0])
        hostI1 = int(hostI.find('"')+1)
        hostI2 = int(len(hostI)-2)
        hostI = hostI[hostI1:hostI2]
        print(hostI)
